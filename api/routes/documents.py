...
def _load_files_parallel(
    ...
    logger.info(
        f"[load_parallel] {n} fichier(s) → {len(all_docs)} chunks ({len(warnings)} ignoré(s))"
    )
    ...
    try:
        ...
        if is_r2_enabled():
            try:
                ...
                logger.warning(
                    f"[BG-reindex] Synchronisation R2 → local ignorée : {e}", exc_info=True
                )
...
def get_documents() -> DocumentsResponse:
    ...
    if not documents:
        raise HTTPException(
            status_code=404, detail="Aucun document indexé. Lance python ingest.py d'abord."
        )
...
def get_categories() -> CategoriesResponse:
    ...
    for key, cfg in cats.items():
        ...
        if directory.exists():
            nb_docs = sum(
                1 for f in directory.iterdir() if f.suffix.lower() in SUPPORTED_EXTENSIONS
            )
        result.append(
            CategoryInfo(
                key=key,
...
def create_category(body: CreateCategoryRequest) -> CategoryInfo:
    ...
    if body.key in cats:
        raise HTTPException(
            status_code=409, detail=f"Une catégorie existe déjà avec l'identifiant « {body.key} »."
        )
...
async def delete_document(
    ...
    return DeleteDocumentResponse(
        ...
        categorie=categorie,
        message=(
            f"Document « {filename} » supprimé. Reconstruction de l'index en cours en arrière-plan."
        ),
        background=True,
    )
